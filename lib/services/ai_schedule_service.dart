...
class AiScheduleService extends ChangeNotifier {
  ...
  Future<void> analyzeSchedule(List<TaskModel> tasks) async {
    ...
    try {
      ...
      2. ### Ranked Task
      Rank which task need attention first.
  ...
  ScheduleAnalysis _parseResponse(String fullText) {
    ...
    for (var section in sections) {
      if (section.startsWith('Detected Conflicts')){
        ...
        recommendedSchedule = section.replaceFirst('Explanation', '').trim();
      }
    }

    return ScheduleAnalysis(conflicts: conflicts, rankedTasks: rankedTasks, recommendedSchedule: recommendedSchedule, explanation: explanation);
  }
}